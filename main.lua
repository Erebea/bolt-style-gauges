bolt = require("bolt")
bolt.checkversion(1, 0)

modules = {
  buffs = require("modules.buffs.buffs"),
}
-- interface scaling value
scale = 1
-- modern or legacy interface, I haven't made legacy assets yet
intstyle = "modern-condensed"
-- the width of the gauge
gw = 340 * scale
-- the height of the gauge
gh = 60 * scale
-- x coordinate of the gauge
gx = (1920 / 2) - (gw /2)
-- y coordinate of the gauge
gy = 600
    -- middle position of the gauge
gm = gx + ( gw / 2)
    --vertical middle of the gauge
gv = gy + (gh / 2)
    -- bottom of the gauge
gb = gy + gh
    -- right of the gauge
gr = gx + gw

cbstyle = "none"
hidegauge = false
intname = intstyle .. '-base'

buffs = {
    -- necromancy
    darkness = {},
    necrosis = {},
    residualsouls = {},
    livingdeath = {},
    splitsoul = {},
    phantomguardian = {},
    vengefulghost = {},
    putridzombie = {},
    skeletonwarrior = {},
    deathmark = {},
    bloat = {},
    threadsoffate = {},
    sorrow = {},
    ruination = {},
    -- magic
    sunshine = {},
    gsunshine = {},
    metamorphosis = {},
    exsanguinate = {},
    incitefear = {},
    glacialembrace = {},
    critbuff = {},
    animatedead = {},
    temporalanomaly = {},
    conflagrate = {},
    corruptionessence = {},
    ruin = {},
    torment = {},
    affliction = {},
    soulfire = {},
    instability = {},
    instabilitycd = {},
    tester = {},
    -- ranged
    anguish = {},
    desolation = {},
    arrowwen = {},
    arrowful = {},
    arrowbik = {},
    arrowjas = {},
    arrowblack = {},
    arrowdspore = {},
    bolthydrix = {},
    boltonyx = {},
    boltruby = {},
    boltdiamond = {},
    boltdstone = {},
    boltemerald = {},
    deathsswiftness = {},
    gdeathsswiftness = {},
    perfectequilibrium = {},
    balancebyforce = {},
    spore = {},
    sporeactive = {},
    sporecd = {},
    wenstack = {},
    wenactive = {},
    sgb = {},
    incen = {},
    draco = {},
    edraco = {},
    -- melee
    gbarge = {},
    turmoil = {},
    malevolence = {},
    berserk = {},
    frostblades = {},
    primordialice = {},
}

models = {
  temporalanomaly = { center = bolt.point(0, 200,0), boxsize = 500, boxthickness = 100},
}

particles = {
  bloat = {},
}

elements = {
  bank = {},
  central = {},
  lobby = {},
  loot = {},
}

for _, model in pairs(models) do
  model.dohighlight = true
end

px = {}
py = {}
pz = {}

-- table of png images ("true" is used as a placeholder value for each one)
images = {
  interface = {
    legacy = { ticker = true, },
    modern = { ticker = true, },
  },
  ['gauge-ui'] = {
    digits = true,
    ['modern-condensed'] = {
      aspects = {
        darkness = {
          active = true,
          },
        ['animate-dead'] = {
          active = true,
        },
        ['temporal-anomaly'] = {
          active = true,
          ['ta-activate'] = true,
        },
      },
      necromancy = {
        digitssmall = true,
        blackbg = true,
        incantations = {
          ['split-soul'] = {
            active = true,
            ssbar = true,
            ssbarbg = true,
            icon = true,
          },
          ['invoke-death'] = {
            active = true,
            inactive = true,
          },
        },
        ['living-death'] = {
          icon = true,
          bg = true,
          digits = true,
          digitsbg = true,
          bar = true,
          barbg = true,
        },
        bloat = {
          bloatbar = true,
          bloatbg = true,
          bloatframe = true,
        },
        necrosis = {
          [0] = true,
          [2] = true,
          [4] = true,
          [6] = true,
          [8] = true,
          [10] = true,
          [12] = true,
        },
        ['residual-souls'] = {
          [0] = true,
          [1] = true,
          [2] = true,
          [3] = true,
          [4] = true,
          [5] = true,
        },
      },
      magic = {
        digitssmall = true,
        blackbg = true,
        ['active-spell'] = {
          ['no-spell'] = true,
          ['exsanguinate'] = true,
          ['incite-fear'] = true,
          ['exsanguinate2'] = true,
          ['incite-fear2'] = true,
          ['ice-barrage'] = true,
          ['blood-barrage'] = true,
        },
        sunshine = {
          sunbar = true,
          icon = true,
          gicon = true,
          iconi = true,
          giconi = true,
        },
        metamorphosis = {
          icon = true,
        },
        tsunami = {
          icon = true,
        },
        ['glacial-embrace'] = {
          ['0'] = true,
          ['5'] = true,
          ready = true,
          gebar = true,
          gebarbg = true,
        },
        ['blood-tithe'] = {
          ['0'] = true,
          ['12'] = true,
          btbar = true,
          btbarbg = true,
        },
        ['corruption-essence'] = {
          ['0'] = true,
          ['1'] = true,
          ['2'] = true,
          ['3'] = true,
          ['0-ready'] = true,
          ['1-ready'] = true,
          ['2-ready'] = true,
          ['3-ready'] = true,
          cebar = true,
          cebarframe = true,
          cebarbg = true,
        },
        instability = {
          icon = true,
        },
      },
      ranged = {
        digits = true,
        digitssmall = true,
        blackbg = true,
        blackbg2 = true,
        blackbg3 = true,
        ammo = {
          wen = true,
          bik = true,
          ful = true,
          jas = true,
          deathspore = true,
          blackstone = true,
          hydrix = true,
          rubyi = true,
          onyx = true,
          diamond = true,
          dragonstone = true,
        },
        ['deaths-swiftness'] = {
          icon = true,
          gicon = true,
          iconi = true,
          giconi = true,
        },
        incen = {
          icon = true,
        },
        ['perfect-equilibrium'] = {
          ['0'] = true,
          ['1'] = true,
          ['2'] = true,
          ['3'] = true,
          ['4'] = true,
          ['5'] = true,
          ['6'] = true,
          ['7'] = true,
          ['8'] = true,
          ['0-bbf'] = true,
          ['1-bbf'] = true,
          ['2-bbf'] = true,
          ['3-bbf'] = true,
          ['4-bbf'] = true,
          deadshot = true,
          snapshot = true,
          ricochet = true,
          rapidfire = true,
          ['deadshot-p'] = true,
          ['snapshot-p'] = true,
          ['ricochet-p'] = true,
          ['rapidfire-p'] = true,
          dot0 = true,
          dot1 = true,
          dot2 = true,
          bar = true,
          barbg = true,
        },
        ['split-soul'] = {
          icon = true,
        },
        bik = {
          digits = true,
          icon = true,
        },
        deathspore = {
          icon = true,
          digits = true,
          frame = true,
          barfill = true,
          barbg = true,
          barfillcd = true,
          ['0'] = true,
          ['1'] = true,
          ['2'] = true,
          ['3'] = true,
          ['4'] = true,
          ['5'] = true,
        },
        wen = {
          icon = true,
          digits = true,
          digitsa = true,
        },
          draco = {
          draco = true,
          edraco = true,
          draco2 = true,
          edraco2 = true,
        },
      },
      melee = {
        digitssmall = true,
        blackbg = true,
        blackbg4 = true,
        lengs = {
          ['0'] = true,
          ['1'] = true,
          ['2'] = true,
          ['3'] = true,
          ['4'] = true,
          ['5'] = true,
          ['6'] = true,
          ['7'] = true,
          ['8'] = true,
          ['9'] = true,
          ['10'] = true,
          barframe = true,
          barbg = true,
          barfill = true,
        },
        berserk = {
          icon = true,
        },
        meteorstrike = {
          icon = true,
        },
        gbarge = {
          icon = true,
        },
      },
    }
  }
}

-- sets up initial states of `images` and `bars`
(function (images)
  local function setupimage (t, prefix)
    for key, val in pairs(t) do
      local str = string.format("%s.%s", prefix, key)
      if type(val) == 'table' then
        setupimage(val, str)
      else
        local surface, width, height = bolt.createsurfacefrompng(str)
        t[key] = { width = width, height = height, surface = surface }
      end
    end
  end
  setupimage(images, "assets")
end)(images)

-- table of bars that may appear on the gauge
bars = {
  necromancy = {
    livingdeath = { max = 3e+7 },
    threadsoffate = { max = 6.6e+6 },
    splitsoul = { max = 2.04e+7 },
    bloat = { max = 2.16e+7 },
  },
  magic = {
    sunshine = { max = 3.06e+7 },
    gsunshine = { max = 3.9e+7 },
    metamorphosis = { max = 1.56e+7 },
    critbuff = { max = 3.06e+7 },
    instability = { max = 3e+7 },
    instabilitycd = { max = 6e+7 },
    incitefear = { max = 2e+7 },
    exsanguinate = { max = 2e+7 },
    corruptionessence = { max = 3e+7 },
  },
  ranged = {
    deathsswiftness = { max = 3.06e+7 },
    gdeathsswiftness = { max = 3.9e+7 },
    critbuff = { max = 3.06e+7 },
    spore = { max = 0.9e+7 },
    sporecd = { max = 6e+7 },
    splitsoul = { max = 1.5e+7 },
    balancebyforce = { max = 3e+7 },
  },
  melee = {
    berserk = { max = 2.04e+7 },
    critbuff = { max = 3.06e+7 },
    frostblades = { max = 0.9e+7},
  },
}

-- updates the "start" variable of each bar according to buffs table, setting it to "t" if it becomes active
updatebarlist = function (bars, t)
  for name, bar in pairs(bars) do
    if buffs[name].active then
      if name == 'spore' then
        if not buffs.spore.number then
          bar.start = nil
        else
          if not buffs.spore.parensnumber then
            if bar.start == nil then
              bar.start = t
            end
          else
            bar.start = nil
          end
        end
      else
        if bar.start == nil then
          bar.start = t
        end
      end
    else
      if name == 'sunshine' or name == 'gsunshine' or name == 'deathsswiftness' or name == 'gdeathsswiftness' or name == 'bloat' then
        if bar.start == nil then
          bar.start = nil
        else
          timer = bars[name].max - (t - bars[name].start)
          if timer <= 0 then
            bar.start = nil
          end
        end
      else
        bar.start = nil
      end
    end
  end
end

numparse = function (num, size)
    local number_to_process
    if type(num) ~= "number" then
        number_to_process = 0
    else
        -- Optional: Cap the number within the supported range (0-99)
        number_to_process = math.max(0, math.min(99, num))
    end

    -- Format the number into a two-digit string (e.g., 5 -> "05")
  local num_str = string.format("%02d", number_to_process)

  -- Return the individual characters
  local digit1_val = tonumber(num_str:sub(1, 1))
  local digit2_val = tonumber(num_str:sub(2, 2))

  -- Calculate the source X coordinate in the image
  local digit1 = digit1_val * size
  local digit2 = digit2_val * size

  return digit1, digit2
end
local function drawbox (worldpoint, viewmatrix, projmatrix, boxradius, boxthickness)
  local px, py, pz = worldpoint:transform(viewmatrix):get()
  local left, top, depth = bolt.point(px - boxradius, py + boxradius, pz):transform(projmatrix):aspixels()
  local right, bottom, _ = bolt.point(px + boxradius, py - boxradius, pz):transform(projmatrix):aspixels()
  local leftinner, topinner, _ = bolt.point(px + boxthickness - boxradius, py + boxradius - boxthickness, pz):transform(projmatrix):aspixels()
  if depth < 0.0 or depth > 1.0 then return end
  left = math.floor(left)
  top = math.floor(top)
  right = math.floor(right)
  bottom = math.floor(bottom)
  local width = right - left
  local height = bottom - top
  local edgew = leftinner - left
  local edgeh = topinner - top
  redpixel:drawtoscreen(0, 0, 1, 1, left, top, width, edgeh) -- top
  redpixel:drawtoscreen(0, 0, 1, 1, left, top, edgew, height) -- left
  redpixel:drawtoscreen(0, 0, 1, 1, right - edgew, top, edgew, height) -- right
  redpixel:drawtoscreen(0, 0, 1, 1, left, bottom - edgeh, width, edgeh) -- bottom
  blackpixel:drawtoscreen(0, 0, 1, 1, left, top, width, 1) -- top
  blackpixel:drawtoscreen(0, 0, 1, 1, left, top, 1, height) -- left
  blackpixel:drawtoscreen(0, 0, 1, 1, right - 1, top, 1, height) -- right
  blackpixel:drawtoscreen(0, 0, 1, 1, left, bottom - 1, width, 1) -- bottom
  blackpixel:drawtoscreen(0, 0, 1, 1, left + edgew, top + edgeh, width - (edgew * 2), 1) -- top
  blackpixel:drawtoscreen(0, 0, 1, 1, left + edgew, top + edgeh, 1, height - (edgeh * 2)) -- left
  blackpixel:drawtoscreen(0, 0, 1, 1, right - (edgew + 1), top + edgeh, 1, height - (edgeh * 2)) -- right
  blackpixel:drawtoscreen(0, 0, 1, 1, left + edgew, bottom - (edgeh + 1), width - (edgew * 2), 1) -- bottom
end

checkframe = false
checktime = bolt.time()
checkinterval = 50000 -- read the screen 20 times per second
nextrender2dbuff = nil
nextrender2ddebuff = nil
nextrender2dpxleft = 0
nextrender2dpxtop = 0

barfill = function (dest, el, max)
  local perc = (dest - el) / dest
  return math.max(0, math.min(max, max * perc))
end

for name, buff in pairs(buffs) do
  buff.name = name
  buff.active = false
end

rgbaleniency = 2.5 / 255.0

-- compares rgb 0-1 to target values using a leniency value
function rougheqrgb (r, g, b, tr, tg, tb)
  return math.abs(r - (tr / 255.0)) <= rgbaleniency and math.abs(g - (tg / 255.0)) <= rgbaleniency and math.abs(b - (tb / 255.0)) <= rgbaleniency
end

any3dobjectexists = false
any3dobjectfound = false

render3dlookup = {
  [600] = function (event)
    -- temporal anomaly
    local anim = event:animated()
    local x, y, z = event:vertexpoint(1):get()
    local a, b, c = event:vertexpoint(89):get()
    local d, e, f = event:vertexpoint(76):get()
    local g, h, i = event:vertexpoint(575):get()
    local j, k, l = event:vertexpoint(340):get()
    local m, n, o = event:vertexpoint(235):get()
    if anim and x == -0 and y == 32 and z == -224 then return models.temporalanomaly end
    if anim and a == 120 and b == 76 and c == -204 then return models.temporalanomaly end
    if anim and d == -192 and e == 280 and f == 112 then return models.temporalanomaly end
    if anim and g == 60 and h == 584 and i == -128 then return models.temporalanomaly end
    if anim and j == -176 and k == 176 and l == 0 then return models.temporalanomaly end
    if anim and m == 112 and n == 460 and o == -188 then return models.temporalanomaly end
    return nil
  end,
}

rendericonlookup1godbookmatch = function (event)
  local x, y, z = event:modelvertexpoint(1, 1):get()
  if x == -7 and y == 67 and z == 81 then return buffs.godbook, nil end
  return nil, nil
end

rendericonlookup1 = {

  [2811] = function (event)
    local x, y, z = event:modelvertexpoint(1, 1):get()
    if x == -76 and y == 35 and z == -27 then return buffs.grimoire, nil end
    return nil, nil
  end,

  [2382] = function (event)
    local x, y, z = event:modelvertexpoint(1, 26):get()
    if x == 0 and y == 5 and z == -202 then return nil, buffs.soulfire end
    return nil, nil
  end,

  [1575] = function (event)
    local x, y, z = event:modelvertexpoint(1, 504):get()
    if x == -144 and y == 0 and z == -156 then return buffs.spore, buffs.sporecd end
  end,

  [2460] = rendericonlookup1godbookmatch, -- wen
  [2484] = rendericonlookup1godbookmatch, -- ful
  [2502] = rendericonlookup1godbookmatch, -- jas
  [2856] = rendericonlookup1godbookmatch, -- elidinis
}

rendericonlookup2 = {
  [24] = function (event)
    -- note: perfect plus and supreme overload have the same icon, so we can't distinguish them.
    -- here we assume the icon is a perfect plus potion since those are far more popular/useful.
    if event:modelvertexcount(2) == 1626 then
      local x, y, z = event:modelvertexpoint(2, 1):get()
      if not (x == -30 and y == 11 and z == -19) then return nil end
      local r, g, b, _ = event:modelvertexcolour(2, 1626)
      if rougheqrgb(r, g, b, 69, 147, 168) then return buffs.overload, nil end -- elder overload icon
      if rougheqrgb(r, g, b, 126, 202, 223) then return buffs.perfectplus, nil end
    end
    return nil, nil
  end,

  [5652] = function (event)
    if event:modelvertexcount(2) == 4014 then return buffs.instability, buffs.instabilitycd end
  end,
}

setbuffdetails = function (buff, isvalid, number, parensnumber)
  if isvalid then
    buff.number = number
    buff.parensnumber = parensnumber
    buff.active = true
    buff.foundoncheckframe = true
  end
end

setuidetails = function (element, exists, x, y)
  if x then
    element.x = x
    element.y = y
    element.active = true
    element.foundoncheckframe = true
  end
end

bolt.onrender2d(function (event)
  local t = bolt.time()
  if not checkframe then return end

  if nextrender2dbuff then
    setbuffdetails(nextrender2dbuff, modules.buffs:tryreadbuffdetails(event, 1, nextrender2dpxleft, nextrender2dpxtop, true))
    nextrender2dbuff = nil
  end
  if nextrender2ddebuff then
    setbuffdetails(nextrender2ddebuff, modules.buffs:tryreadbuffdetails(event, 1, nextrender2dpxleft, nextrender2dpxtop, false))
    nextrender2ddebuff = nil
  end

  local vertexcount = event:vertexcount()
  local verticesperimage = event:verticesperimage()
  for i = 1, vertexcount, verticesperimage do
    local ax, ay, aw, ah, _, _ = event:vertexatlasdetails(i)
    if checkframe then
      local pxleft, pxtop = event:vertexxy(i + 2)
      local readbuff = function (buff, isbuff)
        setbuffdetails(buff, modules.buffs:tryreadbuffdetails(event, i + verticesperimage, pxleft, pxtop, isbuff))
      end
      local readuielement = function (element, exists)
        setuidetails(element, exists, pxleft, pxtop)
      end
      if aw == ah then
        if aw == 60 then
          if event:texturecompare(ax, ay + 30, "") then
                    readbuff(buffs.sunshine, true)
          elseif event:texturecompare(ax, ay + 35, "") then
                    readbuff(buffs.gsunshine, true)
          elseif event:texturecompare(ax, ay + 30, "") then
                    readbuff(buffs.ruin, true)
          elseif event:texturecompare(ax, ay + 20, "") then
                    readbuff(buffs.torment, true)
          elseif event:texturecompare(ax, ay + 30, "") then
                    readbuff(buffs.affliction, true)
          elseif event:texturecompare(ax, ay + 20, "") then
                    readbuff(buffs.anguish, true)
          elseif event:texturecompare(ax, ay + 30, "") then
                    readbuff(buffs.desolation, true)
          elseif event:texturecompare(ax, ay + 20, "") then
                    readbuff(buffs.gdeathsswiftness, true)
          elseif event:texturecompare(ax, ay + 25, "") then
                    readbuff(buffs.deathsswiftness, true)
          elseif event:texturecompare(ax, ay + 25, "") then
                    readbuff(buffs.metamorphosis, true)
          elseif event:texturecompare(ax, ay + 25, "") then
                    readbuff(buffs.berserk, true)
          elseif event:texturecompare(ax, ay + 25, "") then
                    readbuff(buffs.gbarge, true)
          elseif event:texturecompare(ax, ay + 27, "") then
                    readbuff(buffs.turmoil, true)
          elseif event:texturecompare(ax, ay + 30, "") then
                    readbuff(buffs.malevolence, true)
          end
        end
        if aw == 64 then
          if event:texturecompare(ax, ay + 28, "") then
                readbuff(buffs.corruptionessence, true)  
          end
        end
        if aw == 68 then
          if event:texturecompare(ax, ay + 34, "") then
                    readbuff(buffs.balancebyforce, true)
            end
        end
        if aw == 81 then
            if event:texturecompare(ax, ay + 40, "") then
                    readbuff(buffs.exsanguinate, true)
            elseif event:texturecompare(ax, ay + 45, "") then
                readbuff(buffs.incitefear, true)
                readbuff(buffs.glacialembrace, false)
            elseif event:texturecompare(ax, ay + 20, "") then
                    readbuff(buffs.animatedead, true)
            elseif event:texturecompare(ax, ay + 40, "") then
                    readbuff(buffs.wenstack, true)
            elseif event:texturecompare(ax, ay + 25, "") then
                    readbuff(buffs.wenactive, true)
            elseif event:texturecompare(ax, ay + 40, "") then
                    readbuff(buffs.perfectequilibrium, true)
            end
        end
        if aw == 72 then
            if event:texturecompare(ax, ay + 45, "") then
                    readbuff(buffs.conflagrate, true)
            end
        end
        if aw == 120 then
            if event:texturecompare(ax, ay + 50, "") then
                    readbuff(buffs.livingdeath, true)
              elseif event:texturecompare(ax, ay + 60, "") then
                    readbuff(buffs.sorrow, true)
              elseif event:texturecompare(ax, ay + 30, "") then
                    readbuff(buffs.ruination, true)
            end
        end
        if aw == 128 then
            if event:texturecompare(ax, ay + 80, "") then
                    readbuff(buffs.critbuff, true)
            elseif event:texturecompare(ax, ay + 50, "") then
                    readbuff(buffs.temporalanomaly, true)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.darkness, true)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.necrosis, true)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.residualsouls, true)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.splitsoul, true)
            elseif event:texturecompare(ax, ay + 90, "") then
                    readbuff(buffs.bloat, false)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.threadsoffate, true)
            elseif event:texturecompare(ax, ay + 60, "") then
                    readbuff(buffs.deathmark, false)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.skeletonwarrior, true)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.vengefulghost, true)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.putridzombie, true)
            elseif event:texturecompare(ax, ay + 64, "") then
                    readbuff(buffs.phantomguardian, true)
            elseif event:texturecompare(ax, ay + 60, "") then
                    readbuff(buffs.primordialice, true)
            elseif event:texturecompare(ax, ay + 65, "") then
                    readbuff(buffs.frostblades, true)
            end
        end
        if aw == 76 then
          if event:texturecompare(ax, ay + 35, "") then
            readuielement(elements.central, true)
          elseif event:texturecompare(ax, ay + 35, "") then
            readuielement(elements.central, true)
          end
        end
        if aw == 32 then
          if event:texturecompare(ax, ay + 12, "") then
                readbuff(buffs.draco, true)  
          end
        end
      elseif aw == 140 then
        -- checks for bank interface assets
        if event:texturecompare(ax, ay + 16, "") then
          readuielement(elements.bank, true)
        end
      elseif aw == 28 then
        if event:texturecompare(ax, ay + 16, "") then
            readuielement(elements.lobby, true)
        end
      elseif aw == 25 then
        if event:texturecompare(ax, ay + 13, "") then
            readuielement(elements.loot, true)
        end
      end
    end
  end
end)

bolt.onrender3d(function (event)
  if not (checkframe or any3dobjectexists) then return end
  local f = render3dlookup[event:vertexcount()]
  if f ~= nil then
    local model = f(event)
    if model and model.dohighlight then
      any3dobjectfound = true
      if (bolt.time() % 600000) <= 480000 then
        local m = model.center
        if model.anim then
          m = m:transform(event:vertexanimation(1))
        end
        --drawbox(m:transform(event:modelmatrix()), event:viewmatrix(), event:projectionmatrix(), model.boxsize, model.boxthickness)
      end
      if checkframe then
        model.foundoncheckframe = true
      end
    end
  end
end)

bolt.onrenderparticles(function (event)
  local t = bolt.time()
  if not (checkframe or any3dobjectexists) then return end
  for i = 1, event:vertexcount() do
    local viewmatrix = event:viewmatrix()
    local projmatrix = event:projmatrix()
    local viewproj = event:viewprojmatrix()
    
    -- the world position of this particle
    local x, y, z = event:vertexparticleorigin(i):get()

    -- world offset of this individual vertex of the particle
    local wx, wy, wz = event:vertexworldoffset(i)

    -- eye offset of this individual vertex of the particle
    local ex, ey = event:vertexeyeoffset(i)

    -- apply transforms and offsets in the correct order:
    local eyepoint = bolt.point(x + wx, y + wy, z + wz):transform(viewmatrix)
    local eyex, eyey, eyez = eyepoint:get()
    local screenpoint = bolt.point(eyex + ex, eyey + ey, eyez):transform(projmatrix)
    local xpixel, ypixel, depth = screenpoint:toscreen()
    -- color
    local r, g, b, _ = event:vertexcolour(i)
    --local ax, ay, aw, ah, _, _ = event:vertexatlasdetails(i)
    local meta = event:vertexmeta(1)
    local ax, ay, aw, ah = event:atlasxywh(meta)
    if checkframe then
      if aw == ah then
        if aw == 512 then
          if event:texturecompare(ax, ay + 132, "") then
            local playerposition = bolt.playerposition()
            local px, py, pz = playerposition:get()
            local rx = math.abs(px - x)
            local ry = math.abs(py - y)
            local rz = math.abs(pz - z)
            if ry <= 20 and buffs.bloat.active then
                if rougheqrgb(r, g, b, 38, 20, 67) or rougheqrgb(r, g, b, 55, 16, 147) or rougheqrgb(r, g, b, 38, 20, 66) or rougheqrgb(r, g, b, 16, 22, 0) then
                particles.bloat.foundoncheckframe = true
                end
            end
          end
        end
      end
    end
  end
end)

bolt.onrendericon(function (event)
  if not checkframe then return end
  local modelcount = event:modelcount()
  local f = nil
  if modelcount == 1 then
    f = rendericonlookup1[event:modelvertexcount(1)]
  elseif modelcount == 2 then
    f = rendericonlookup2[event:modelvertexcount(1)]
  end
  if f ~= nil then
    nextrender2dbuff, nextrender2ddebuff = f(event)
    nextrender2dpxleft, nextrender2dpxtop, _, _ = event:xywh()
  end
end)

startcheckframe = function (t)
  for _, buff in pairs(buffs) do
    buff.foundoncheckframe = false
  end
  for _, model in pairs(models) do
    model.foundoncheckframe = false
  end
  for _, element in pairs(elements) do
    element.foundoncheckframe = false
  end
  for _, particle in pairs(particles) do
    particle.foundoncheckframe = false
  end
end

endcheckframe = function (t)
  for _, buff in pairs(buffs) do
    if not buff.foundoncheckframe then
      buff.active = false
    end
  end
  for _, element in pairs(elements) do
    if not element.foundoncheckframe then
        element.active = false
    end
  end
end

bolt.onswapbuffers (function (event)
  t = bolt.time()
  if checkframe then
    endcheckframe(t)
    checkframe = false
  end

  if t > checktime + checkinterval then
    checkframe = true
    checktime = checktime + checkinterval
    if checktime < t then
      checktime = t
    end
    startcheckframe(t)
  end
  if (elements.bank.active and not elements.loot.active) or (elements.central.active) or (elements.lobby.active) then
    return
  else
    if buffs.livingdeath.active or buffs.sorrow.active or buffs.ruination.active or buffs.skeletonwarrior.active then
      cbstyle = "necromancy"
    elseif buffs.sunshine.active or buffs.gsunshine.active or buffs.torment.active or buffs.affliction.active or buffs.metamorphosis.active then
      cbstyle = "magic"
    elseif buffs.deathsswiftness.active or buffs.gdeathsswiftness.active or buffs.anguish.active or buffs.desolation.active then
      cbstyle = "ranged"
    elseif buffs.berserk.active or buffs.turmoil.active or buffs.malevolence.active then
      cbstyle = "melee"
    end
    if cbstyle == "none" then
      return
    elseif cbstyle == "necromancy" then ----------------------------------------------- necromancy
      require("gauges.necromancy_" .. intstyle)
      drawnecrogauge()

    elseif cbstyle == "magic" then ---------------------------------------------------- magic
      require("gauges.magic_" .. intstyle)
      drawmagegauge()

    elseif cbstyle == "ranged" then ---------------------------------------------------- magic
      require("gauges.ranged_" .. intstyle)
      drawrangedgauge()

    elseif cbstyle == "melee" then ---------------------------------------------------- magic
      require("gauges.melee_" .. intstyle)
      drawmeleegauge()
    end
--this is the end for the onswapbuffers event
end
end)